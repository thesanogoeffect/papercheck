...
We start by searching for a link to the Open Science Framework (OSF) by using "osf_check" module. This can let you know if a link is open, closed, view-only, or if there is an error with the URL.

```{r search_osf}
#paper <- psychsci$`09567976241279291`
paper <- psychsci$`0956797620955209`
osf_links <- module_run(paper, "osf_check")
```
...
```{r enrich_links}
link_info <- osf_retrieve(osf_links$table$text)
```
...
There are multiple OSF links in this paper, but they are all part of the same overarching OSF project, with the project ID *`r link_info$osf_id[[1]]`*. 

### Get all OSF Contents

The function `osf_contents()` will loop over all files and folders in this project (and in all other projects, if a manuscript points to multiple overarching IDs), and retrieve all available information about the files in each project, such as file names, file paths, project titles, folder names, and the wiki.

If you give this function a list with duplicate projects, it will only get the contents for each project once. 

```{r get_osf_files}
osf_files <- osf_contents(link_info$project)
```
...
#| echo: false
osf_files |>
  select(osf_id, name, type, category, filetype) |>
  mytable()
```
...
osf_report <- function(summary) {
  files <- dplyr::filter(summary, type == "file")
...
```{r second_paper}
papercheck::verbose(FALSE) # quiets progress messages
