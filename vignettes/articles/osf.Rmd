...
vignette: >
  %\VignetteIndexEntry{OSF Functions}
...
We start by searching for OSF URLs using the `search_text()` function. 
...
The function `osf_retrieve()` will also retrieve all child components, files and folders if you set the argument `recursive = TRUE`. We can retrieve all of the contents under the parent project(s) with the following code. If there are duplicate IDs, it will only get the contents for each item once. 
